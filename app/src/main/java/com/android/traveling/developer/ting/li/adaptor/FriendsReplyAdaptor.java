...
public class FriendsReplyAdaptor extends RecyclerView.Adapter<FriendsReplyAdaptor.FriendsReplyViewHolder> {
   ...
   // private int commentId; //所属评论的评论id
    ...
    public FriendsReplyAdaptor(Context context, int id,  List<Reply> replyList) {
    ...
    @NonNull
    @Override
    public FriendsReplyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_comment2, parent, false);
        return new FriendsReplyViewHolder(view);
    }
    @Override
     ...
     * @param reply  reply
     */
    private void addEvents(@NonNull FriendsReplyViewHolder holder, Reply reply, int position) {
        //头像/用户名点击
        ...
        holder.tv_reply.setOnClickListener(v -> {
            ...
            new ReplyDialog(context, hint, (v1, content) -> {
                //回复
                BaseComment baseComment = new BaseComment(Reply.FLAG_COMPANION_COMMENT, id,
    ...
    private SpannableString disPoseContent(Reply reply) {
        if (reply.getFlag() == Reply.FLAG_COMPANION_COMMENT) {
    ...
    static class FriendsReplyViewHolder extends RecyclerView.ViewHolder {
        ...
        FriendsReplyViewHolder(View itemView) {
            super(itemView);
