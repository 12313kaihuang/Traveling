...
public class DateUtils {
    ...
    public static long string2Lone(String date, SimpleDateFormat format) {
        try {
     ...
     * @see #fromNow(String)
     */
    public static String fromNow(long timeMillis) {
        long timeInMillis = Calendar.getInstance().getTimeInMillis();

        //兼容脏数据。抓取的数据有些帖子的时间戳不是标准的十三位
        String valueOf = String.valueOf(timeMillis);
        if (valueOf.length() < 13) {
            timeMillis = timeMillis * 1000;
        }
        long diff = (timeInMillis - timeMillis) / 1000;
        if (diff <= 5) {
            return "刚刚";
        } else if (diff < 60) {
            return diff + "秒前";
        ...
        return fromNow(transform(new Date(timeMillis)));
    }
    ...
    public static String fromNow(String createTime) {
        ...
        if (isThisYear(cTime)) {
            if (android.text.format.DateUtils.isToday(cTime.getTime())) {
